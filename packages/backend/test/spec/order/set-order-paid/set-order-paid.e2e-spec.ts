...
describe('Update MentoringSlot Category (e2e)', () => {
  ...
  it('should order status', async () => {
    ...
    const getAllOrdersByCustomerResponse = await request(app.getHttpServer()).patch(`/api/orders/${order.id}/pay-order`);
    expect(getAllOrdersByCustomerResponse.status).toBe(200);
    expect(getAllOrdersByCustomerResponse.body.status).toEqual(OrderStatusEnum.Paid);
  });
